$(document).on "turbolinks:load", ->
    $(".ui.labeled.icon.right.inline.vertical.sidebar.menu").css({"padding-top": $("#aboutNavbar").height() });
    $(window).on 'scroll': ->
        if($(window).scrollTop() >= 20)
            $("#leftNavbar").attr 'style', 'height:90px; width:90px'
            $(".ui.labeled.icon.right.inline.vertical.sidebar.menu").css({"padding-top": $("#aboutNavbar").height() + 36 });
